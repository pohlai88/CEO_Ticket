...
try {
  ...
  if (glossary._meta.authoritative !== false) {
    throw new Error('Glossary _meta.authoritative must be false (glossary is derived, not authoritative)');
  }
  ...
  console.log(`   - ${allFields.length} fields defined (${sourceBindings} with source bindings)`);
  console.log(`   - ${glossary.concepts.length} concepts defined (${conceptSourceBindings} with source bindings)`);
  process.exit(0);
} catch (error) {
