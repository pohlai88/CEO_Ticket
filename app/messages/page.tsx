...
import { Plus, AlertCircle, CheckCircle2, Clock, Zap } from 'lucide-react';
...
export default function MessagesPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'draft' | 'sent' | 'acknowledged' | 'resolved'>('all');
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {(['all', 'draft', 'sent', 'acknowledged', 'resolved'] as const).map((f) => (
            <Button
              key={f}
            ...
            {messages.map((msg) => (
              <Card
                ...
                className={`p-4 cursor-pointer hover:shadow-lg transition-shadow ${
                  !msg.is_read && !msg.current_user_is_author ? 'border-l-4 border-l-blue-500' : ''
                }`}
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <Badge className={`flex-shrink-0 capitalize ${getStatusColor(msg.status)}`}>
                      ...
                      {msg.recipient_ids.length > 0 && (
                        <>
                          ...
                          <span>
                            to{' '}
                            <span className="font-medium text-gray-700">
                              {msg.recipient_ids.length} recipient{msg.recipient_ids.length !== 1 ? 's' : ''}
                            </span>
                          </span>
                        </>
                      )}
