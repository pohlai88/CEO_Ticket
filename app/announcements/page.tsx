...
export default function AnnouncementsPage() {
  ...
  async function checkUserRole() {
    try {
      // Simple role check - in production, fetch from auth context
      // For now, assume CEO if they can access /announcements/create
      const res = await fetch('/api/announcements');
      if (res.ok) {
        // Check if user has CEO role (simplified)
        setIsCEO(true); // TODO: Implement proper role check
      }
    } catch (error) {
  ...
  const urgentOutstanding = announcements.filter(a => a.is_urgent_outstanding);
  const important = announcements.filter(a => a.announcement_type === 'important' && !a.is_urgent_outstanding);
                  ...
                  <Card key={announcement.id} className="border-red-500 border-2">
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        ...
                        <Badge variant={getTypeBadge(announcement.announcement_type)}>
