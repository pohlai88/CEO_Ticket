...
describe('FSM Status Transitions (RCF-4)', () => {
  ...
  describe('Forbidden Transitions (Terminal States)', () => {
    it('CANCELLED cannot transition to anything', () => {
      const allStatuses: RequestStatus[] = ['DRAFT', 'SUBMITTED', 'IN_REVIEW', 'APPROVED', 'REJECTED', 'CANCELLED', 'CLOSED'];
...
describe('Status Schema Validation', () => {
  it('accepts all valid statuses', () => {
    const validStatuses = ['DRAFT', 'SUBMITTED', 'IN_REVIEW', 'APPROVED', 'REJECTED', 'CANCELLED', 'CLOSED'];
  ...
  it('rejects invalid statuses', () => {
    const invalidStatuses = ['PENDING', 'ACTIVE', 'DONE', 'draft', 'approved', ''];
