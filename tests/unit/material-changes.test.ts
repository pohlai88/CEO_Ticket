  ...
  describe('isMaterialChange()', () => {
    describe('Material Changes (should return true)', () => {
      it('detects title change', () => {
        const old = { title: 'Old title', description: null, priority_code: 'P3', category_id: null };
        const updated = { title: 'New title', description: null, priority_code: 'P3', category_id: null };
        expect(isMaterialChange(old, updated)).toBe(true);
      });

      it('detects description change', () => {
        const old = { title: 'Title', description: 'Old description', priority_code: 'P3', category_id: null };
        const updated = { title: 'Title', description: 'New description', priority_code: 'P3', category_id: null };
        expect(isMaterialChange(old, updated)).toBe(true);
      });

      it('detects priority_code change', () => {
        const old = { title: 'Title', description: null, priority_code: 'P3', category_id: null };
        const updated = { title: 'Title', description: null, priority_code: 'P1', category_id: null };
        expect(isMaterialChange(old, updated)).toBe(true);
      });

      it('detects category_id change', () => {
        const old = { title: 'Title', description: null, priority_code: 'P3', category_id: null };
        const updated = { title: 'Title', description: null, priority_code: 'P3', category_id: 'uuid-123' };
        expect(isMaterialChange(old, updated)).toBe(true);
      });

      it('detects description change from null to value', () => {
        const old = { title: 'Title', description: null, priority_code: 'P3', category_id: null };
        const updated = { title: 'Title', description: 'Now has description', priority_code: 'P3', category_id: null };
        expect(isMaterialChange(old, updated)).toBe(true);
      });

      it('detects description change from value to null', () => {
        const old = { title: 'Title', description: 'Has description', priority_code: 'P3', category_id: null };
        const updated = { title: 'Title', description: null, priority_code: 'P3', category_id: null };
        expect(isMaterialChange(old, updated)).toBe(true);
      });
    });
    ...
    describe('Non-Material Changes (should return false)', () => {
      it('returns false when no changes', () => {
        const old = { title: 'Title', description: 'Desc', priority_code: 'P3', category_id: 'uuid-123' };
        const updated = { title: 'Title', description: 'Desc', priority_code: 'P3', category_id: 'uuid-123' };
        expect(isMaterialChange(old, updated)).toBe(false);
      });

      it('returns false when only non-material fields change', () => {
        const old = { title: 'Title', description: null, priority_code: 'P3', category_id: null, watchers: [] };
        const updated = { title: 'Title', description: null, priority_code: 'P3', category_id: null, watchers: ['user-1'] };
        expect(isMaterialChange(old, updated)).toBe(false);
      });
    });
  });
