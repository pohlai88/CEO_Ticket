  ...
  describe('Valid Inputs', () => {
    ...
    it('accepts full valid request', () => {
      const input = {
        ...
        description: 'We need to expand the platform team to handle increased workload.',
        priority_code: 'P2',
...
describe('transitionStatusSchema', () => {
  describe('Valid Inputs', () => {
    ...
    it('accepts all valid target statuses', () => {
      const validStatuses = ['SUBMITTED', 'IN_REVIEW', 'APPROVED', 'REJECTED', 'CANCELLED', 'CLOSED'];
      for (const status of validStatuses) {
        const result = transitionStatusSchema.safeParse({ target_status: status });
        expect(result.success).toBe(true);
      }
    });
  });
