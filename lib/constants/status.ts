...
/**
 * Material change fields - edits to these fields invalidate active approvals
 * Synced from prd-guard canonical.ts v2.2.0
 */
...
/**
 * Finite State Machine transitions - valid status transitions only
 * Synced from prd-guard canonical.ts v2.2.0
 */
...
// Status lifecycle transitions (derived from canonical FSM_TRANSITIONS)
// Type assertion needed because canonical uses readonly arrays for immutability
export const STATUS_TRANSITIONS = FSM_TRANSITIONS as unknown as Record<
  RequestStatus,
  RequestStatus[]
>;
...
export const DEFAULT_PRIORITY_METADATA: Record<
  PriorityCode,
  {
    label: string;
    color: string;
